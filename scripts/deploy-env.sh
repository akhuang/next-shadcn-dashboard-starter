...
# 多环境部署脚本 - Next.js Dashboard with Nginx
# 使用方法: ./deploy-env.sh [环境] [选项]
...
show_help() {
    ...
    echo ""
    echo "环境:"
    echo "  dev                 开发环境 (默认, 端口 8089/8445)"
    echo "  prod                生产环境 (端口 8088/8444)"
    echo ""
    echo "选项:"
    echo "  -h, --help          显示帮助信息"
    ...
    echo ""
    echo "示例:"
    echo "  $0 dev -b -u        开发环境：构建并启动"
    echo "  $0 prod -b -u       生产环境：构建并启动"
    echo "  $0 dev -s           查看开发环境状态"
    echo "  $0 prod -s          查看生产环境状态"
    echo ""
    echo "端口分配:"
    echo "  开发环境: HTTP=8089, HTTPS=8445"
    echo "  生产环境: HTTP=8088, HTTPS=8444"
}
    ...
    case "$1" in
        ...
        *)
            ...
            if [[ "$1" == -* ]] || [ -z "$1" ]; then
                ...
                return 1  # 返回1表示没有消费这个参数
            else
                print_message "❌ 未知环境: $1" "$RED"
                echo "有效的环境: dev, prod"
                exit 1
            fi
            ;;
    esac
    return 0  # 返回0表示成功消费了参数
}
...
# 构建镜像
build_images() {
    print_message "🔨 构建 Docker 镜像 ($ENV 环境)..." "$BLUE"
...
start_containers() {
    ...
    if [ "$ENV" = "dev" ]; then
        ...
        print_message "\n📌 生产环境访问地址:" "$GREEN"
        print_message "   HTTP:  http://localhost:$HTTP_PORT" "$YELLOW"
        print_message "   HTTPS: https://localhost:$HTTPS_PORT" "$YELLOW"
    fi
}
...
# 重新加载 Nginx 配置
reload_nginx() {
    print_message "🔄 重新加载 Nginx 配置 ($ENV 环境)..." "$BLUE"
    
    container_name="nextjs-dashboard-nginx"
    if [ "$ENV" = "dev" ]; then
...
show_all_status() {
    ...
    print_message "   生产: http://localhost:8088 | https://localhost:8444" "$YELLOW"
}
...
main() {
    ...
    # 处理第一个参数（可能是环境或选项）
    if set_environment "$1"; then
        shift  # 如果成功设置了环境，移除第一个参数
    fi

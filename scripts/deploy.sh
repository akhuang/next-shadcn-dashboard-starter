...
build_images() {
    ...
    # 预先生成 SSL 证书
    if [ -f "./prepare-certs.sh" ]; then
        print_message "准备 SSL 证书..." "$YELLOW"
        ./prepare-certs.sh "$SERVER_IP"
    elif [ ! -f "./certs/portal.crt" ] || [ ! -f "./certs/portal.key" ]; then
        print_message "生成自签名 SSL 证书..." "$YELLOW"
        if [ -f "./generate-certs.sh" ]; then
            ./generate-certs.sh localhost 3650 "$SERVER_IP"
        fi
    fi
